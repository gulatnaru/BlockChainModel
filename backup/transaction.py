...
class TxIn:
    ...
    def __str__(self):
        return json.dumps(self, default=self.dict_s, sort_keys = True) #문자열임. 정렬
...
class Transaction:
    ...
    def validate_chk(self):
        ...
        for input in self.inputs:
            if sha256(input.pubk.encode()) == input.pubk_hs:
                ...
                raise Exception
            # sign 검증
            if ecdsa_verify(str(self.gen_indata()), input.sign, input.pubk):
                print('sign OK!')
            else:
                print('sign hacking!!')
                raise Exception
    ...
    def gen_indata(self):
        for input in self.inputs:
            return json.dumps({'hash':input.hash, 'n':input.n, 'address':input.address, 'value':input.value})
    ...
    def gen_sign(self, privk):
        ...
        for input in self.inputs:
            ...
            input.sign = ecdsa_sign(self.gen_indata(), privk)
